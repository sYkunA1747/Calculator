...
double evaluateRPN(const Token *rpnTokens, int rpnCount, ErrorCode *err){
    ...
    for(int i = 0; i < rpnCount; i++){
        if(rpnTokens[i].type == TOKEN_NUMBER){
            ...
            switch(rpnTokens[i].type){
                case TOKEN_COS:     numStack[top] = CalculateCos(numStack[top]); break; 
                case TOKEN_SIN:     numStack[top] = CalculateSin(numStack[top]); break; 
                case TOKEN_COT:     numStack[top] = CalculateCot(numStack[top]); break; 
                case TOKEN_TAN:     numStack[top] = CalculateTan(numStack[top]); break;

                case TOKEN_ARCCOS:  numStack[top] = CalculateArcCos(numStack[top]); break; 
                case TOKEN_ARCSIN:  numStack[top] = CalculateArcSin(numStack[top]); break; 
                case TOKEN_ARCCOT:  numStack[top] = CalculateArcCot(numStack[top]); break; 
                case TOKEN_ARCTAN:  numStack[top] = CalculateArcTan(numStack[top]); break; 
                ...
                case TOKEN_NTHROOT_FUNCTION:
                    ...
                    numStack[top-1] = CalculateNthRoot(numStack[top-1],numStack[top]); 
                    top--; 
                    break;
