...
static bool tokenizeVariable(Context* context) {
  ...
  char* endPtr;
  context->tokens[count].Val.number =
      strtod(&context->str[context->index], &endPtr);
...
static bool tokenizeOperator(Context* context) {
  ...
  if (type == TOKEN_MINUS) {
    if (context->count == 0) type = TOKEN_UNAR_MINUS;
    if (context->count > 0) {
      TypeToken lastToken = context->tokens[context->count - 1].token;
      if (lastToken == TOKEN_OPEN_PAREN || isOperator(lastToken))
        type = TOKEN_UNAR_MINUS;
    }
  }
...
static bool tokenizeFunction(Context* context) {
  ...
  while (isalpha((unsigned char)context->str[context->index]) &&
         len < MAX_TOKEN_LEN - 1) {
  ...
  if (type == TOKEN_UNKNOWN) type = TOKEN_VARIABLE;
  if (type == TOKEN_UNKNOWN) return false;
  context->tokens[context->count].token = type;
  if (type == TOKEN_VARIABLE)
    strcpy(context->tokens[context->count].Val.name, buffer);
  context->count++;
  return true;
}
...
bool dispatchState(Context* context) {
  switch (context->type) {
    case STATE_SPACE_HANDLING:
      return tokenizeSpace(context);
    case STATE_VARIABLE_HANDLING:
      return tokenizeVariable(context);
    case STATE_OPERATOR_HANDLING:
      return tokenizeOperator(context);
    case STATE_FUNCTION_HANDLING:
      return tokenizeFunction(context);
    case STATE_ERROR_HANDLING:
      return tokenizeError(context);
    default:
      return false;
  }
}
...
static States determineNextState(Context* ctx) {
  unsigned char ch = (unsigned char)ctx->str[ctx->index];
  if (isspace(ch)) return STATE_SPACE_HANDLING;
  if (isdigit(ch) || ch == '.') return STATE_VARIABLE_HANDLING;
  if (getOperatorFunctionType(ch) != TOKEN_UNKNOWN)
    return STATE_OPERATOR_HANDLING;
  if (isalpha(ch)) return STATE_FUNCTION_HANDLING;
  return STATE_ERROR_HANDLING;
}